...
$| = 1;
my ($file1,  $file2,     $paired_files, $prefix, $ref_file,
...
if ( $step_size > $window_size ) {
    die
        "The step_size ($step_size) should be less than window_size ($window_size)\n&Usage";
}
...
my $samtools_threads
    = $bwa_threads || $bowtie_threads || $snap_threads || "1";
...
if ($paired_files) {
    ...
    if ( $offset == 64 ) {
        $bowtie_options = $bowtie_options . " --phred64 ";
...
foreach my $ref_name ( sort keys %{$ref_hash} ) {
    ...
    $stats_print_string
        .= &window_size_coverage( $coverage_output, $WindowCoverage_output,
        \%base_hash, $gap_output, $ref_name, $ref_len );
...
# return statistiacl numbers, genome recovery, fold coveage, fold coverage std, gap number, gap total bases.
    my ( $coverage_output, $WindowCoverage_output, $base_hash, $gap_output,
        $ref_name, $ref_len )
        = @_;
    my $pos_cov;
    my $cov_sum;
    ...
    open( OUT2, ">$WindowCoverage_output" )
        ...
        if ( $base_hash->{$ref_name}->{$_} ) {
            ...
            if ($coverage_output) {
                ...
                if (@gap_array) {
                    ...
                    print GAP $gap_array[0], "\t", $gap_array[-1], "\t",
    ...
    if ($coverage_output) {
        ...
        my $percent_genome_coverage
            = sprintf( "%.2f%%", $covered_base_num / $ref_len * 100 );
